...
Ena=`ps -efl | grep trace_mgr`
Enable=`echo $Ena | grep start_link`

if [ -z "$Enable" ] ; then 
    echo "FAILEDJOB - NSS Logging is not enabled">>/netsim/netsimdir/res.txt
else
    echo "NSS Logging is being enabled">>/netsim/netsimdir/res.txt
fi
...
rm -rf /netsim/health_check-15k_17.6.sh
rm -rf /netsim/netsim_checklist-15k_17.6.sh
rm -rf /netsim/compare-15k_17.6.sh
rm -rf /netsim/Mocnt-15k_17.6.sh
rm -rf SIML1.txt SIML2.txt Cron1.txt Cron2.txt celltrace1.txt celltrace2.txt LoadRead.txt LoadRef.txt Pread.txt Pref.txt
