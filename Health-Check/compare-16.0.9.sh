...
then
    ...
    echo "FAILEDJOB -There are less number of started nodes than required:$STNE">>/netsim/netsimdir/res.txt
fi
...
case $Days in
    ...
    *)
	if [ $Days -ge 10 ]
	then
	    echo "FAILEDJOB - Uptime is more than 10day pls reboot the machine:Days=$Days">>/netsim/netsimdir/res.txt
	else
	    echo "Uptime is fine">>/netsim/netsimdir/res.txt
	fi
	;;
...
sed -n '/Celltrace and UETRACE file generation/,/CellTrace for MMEs/p' /netsim/$1 >>celltrace1.txt
...
then
    ...
    echo "FAILEDJOB Mismatch in the simulation count:$Simcount1">>/netsim/netsimdir/res.txt
fi
...
then
    ...
    echo "FAILEDJOB -Invalid NETSim Version:$Version1">>/netsim/netsimdir/res.txt
fi
...
rm -rf /netsim/health_check-16.0.9.sh
rm -rf /netsim/netsim_checklist-16.0.9.sh
rm -rf /netsim/compare-16.0.9.sh
rm -rf /netsim/Mocnt-16.0.9.sh
rm -rf SIML1.txt SIML2.txt Cron1.txt Cron2.txt celltrace1.txt celltrace2.txt LoadRead.txt LoadRef.txt Pread.txt Pref.txt
