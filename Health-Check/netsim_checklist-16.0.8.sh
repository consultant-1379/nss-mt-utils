...
echo "Testingyang"
#if [ ! -n "$1" ] ; then
 #   echo "Invalid Catalog"
  #  exit
#else
#	echo "Valid Catalog"
	#w3m -dump -no-proxy https://arm901-eiffel004.athtem.eei.ericsson.se:8443/nexus/content/repositories/simnet/com/ericsson/simnet/netsim/patches/$1/|grep -o #P0.*|awk '{print $1}' >>/netsim/netsimdir/ReadMe.txt
#fi 
...
Sim_Count=`ls -1 /netsim/netsimdir/*/simulation.netsimdb | sed -e "s/.simulation.netsimdb//g" -e "s/^[^*]*[*\/]//g" |grep -v -E default |wc`
