...
then
    ...
    echo "FAILEDJOB -There are less/More number of started nodes than required:$STNE
         [Suggestion]:Could you cross verify with ESC page.">>/netsim/netsimdir/res.txt
fi
...
cat /netsim/netsimdir/ReadMe.txt|grep "Node level load configuration"|sed 's/[ \t]*$//' >>LoadRead.txt
cat /netsim/$1|grep "Node level load configuration"|sed 's/[ \t]*$//'>>LoadRef.txt
...
jsonFile=`ls /netsim/netsimdir/ |grep networkMap.json`
if [ !  -z  $jsonFile ]  && [ $jsonFile = "networkMap.json" ]
then
echo "Json File is available">>/netsim/netsimdir/res.txt
else
echo "FAILEDJOB -There is no networkMap.json file exists">>/netsim/netsimdir/res.txt
fi


echo -e "\n"
echo -e "\033[1;34m ##################Job Result##################################\033[0m"
echo "Health Check script output"
cat /netsim/netsimdir/res.txt
echo -e "\n"
echo -e "\033[1;34m ############End of the Job with Result##########################\033[0m"
rm -rf /netsim/health_check-17.0.4.sh
rm -rf /netsim/netsim_checklist-17.0.4.sh
rm -rf /netsim/compare-17.0.4.sh
rm -rf /netsim/Mocnt-17.0.4.sh
rm -rf SIML1.txt SIML2.txt Cron1.txt Cron2.txt celltrace1.txt celltrace2.txt LoadRead.txt LoadRef.txt Pread.txt Pref.txt
